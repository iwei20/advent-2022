...
westmost = min(elves, key=lambda elf: elf[1])
eastmost = max(elves, key=lambda elf: elf[1])
northmost = min(elves, key=lambda elf: elf[0])
southmost = max(elves, key=lambda elf: elf[0])

print((southmost[0] - northmost[0] + 1) * (eastmost[1] - westmost[1] + 1) - len(elves)) 